...
async function getSuggestions(userId) {
  ...
  const refinedMatches = await Promise.all(basicMatches.map(async (user) => {
    ...
    if (userProfile && currentUserProfile) {
      ...
      if (userProfile.industry === currentUserProfile.industry) {
        score += 1;
      }
      if (userProfile.education.some(edu => currentUserProfile.education.some(ce => ce.fieldOfStudy === edu.fieldOfStudy))) {
        score += 1;
      }
    }
    return { user, score };
  ...
  if (filteredMatches.length > 0) {
    ...
    // Fallback to random users if no matches are found
